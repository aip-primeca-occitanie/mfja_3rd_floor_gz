...
def segments(module, lengths):
    transitions = module.config["graph"]["transitions"]
    names = [
        *transitions["pick"],
...
def test_execution_plan_uses_semantic_hpp_exec_segments(monkeypatch):
    planning = load_planning_module(monkeypatch)
    transitions = planning.config["graph"]["transitions"]
    planned = [
    ...
    assert [
        ...
        (0, 2, transitions["pick"][0]),
        (2, 4, transitions["grasp"]),
        (4, 6, transitions["detach"]),
    ]
