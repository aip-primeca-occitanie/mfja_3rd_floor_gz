...
The earlier full-evidence release assets described above can be downloaded and
verified without adding their payload to Git. These commands require
\code{curl}, GNU \code{sha256sum}, GNU \code{tar} with Zstandard support and
the \code{zstd} executable:

\begin{lstlisting}[language=bash]
TAG=room315-visual-v4-evidence-2026-08-11
BASE=https://github.com/aip-primeca-occitanie/mfja_3rd_floor_gz/releases/download/$TAG
curl -fLO "$BASE/room315_visual_v4_full_evidence_2026-08-11.tar.zst"
curl -fLO "$BASE/room315_visual_v4_full_evidence_2026-08-11.tar.zst.sha256"
sha256sum -c \
...
\paragraph{Data and replay release.}
The organisation-owned
\href{https://github.com/aip-primeca-occitanie/mfja_3rd_floor_gz}{source project}
publishes the
\href{https://github.com/aip-primeca-occitanie/mfja_3rd_floor_gz/releases/tag/v4-seed31520260811-dataset-v1}{data and replay release
\code{v4-seed31520260811-dataset-v1}}. Its tag targets revision
\path{a26dbd5569b5bb1ab4f794f96fbbd3e8486aca30}, and GitHub records
\code{published\_at} as \path{2026-08-16T18:40:57Z}. The tag is treated as
immutable: later documentation records this target rather than retargeting it.
The release contains the four labelled roles used by the reported procedure:
5,528 Training, 512 Validation, 256 Development-Canary and 1,040 Final-Test
scenes. Thus \(5{,}528+512+256+1{,}040=7{,}336\) paired-camera scenes account
for 14,672 images. It also retains the predecessor V3 initialisation checkpoint
(SHA-256
\path{8a2d865e3d3551ec4284b53aa913d66f24640e23556f2f26b49a165f3ce8d51d}),
the selected V4 epoch-11 checkpoint identified above, the frozen evaluation
source and a portable stateless replay program.

These materials support repeating the documented procedure and re-evaluating
the selected checkpoint on Validation, Canary and Final Test. They do not claim
bit-identical retraining across arbitrary hardware/software environments.
Public accessibility is a reproduction-delivery property, not an ``open
dataset'' designation or a standalone reuse licence. Publication occurred only
after training, checkpoint selection, Canary evaluation and the immutable
Final-Test attempt had completed; it therefore changes availability, not any
partition's experimental role, and the stateless replay does not reopen the
historical one-shot attempt.

Prerequisites for anonymous download and verification are Git, \code{curl}, GNU
\code{sha256sum}, \code{tar} with Zstandard support and Python~3:

\begin{lstlisting}[language=bash]
set -eu
DATA_TAG=v4-seed31520260811-dataset-v1
TARGET=a26dbd5569b5bb1ab4f794f96fbbd3e8486aca30
PROJECT_URL=https://github.com/aip-primeca-occitanie/mfja_3rd_floor_gz
git clone --branch "$DATA_TAG" --depth 1 \
  "$PROJECT_URL.git" room315-v4-release
cd room315-v4-release
test "$(git rev-parse HEAD)" = "$TARGET"
cd report/evidence/room315_visual_v4_dataset_release_v1
mkdir -p release-assets
while read -r _digest asset; do
  curl -fL --retry 3 -o "release-assets/$asset" \
    "$PROJECT_URL/releases/download/$DATA_TAG/$asset"
done < SHA256SUMS
(cd release-assets && sha256sum --strict -c ../SHA256SUMS)
\end{lstlisting}

The data-release inventory and the \code{tar --zstd} extraction and stateless
replay commands are in
\path{report/evidence/room315_visual_v4_dataset_release_v1/README.md}. Matching
the recorded full CUDA replay setup additionally requires the environment in
