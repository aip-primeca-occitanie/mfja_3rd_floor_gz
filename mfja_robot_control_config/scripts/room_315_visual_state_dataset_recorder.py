...
VISUAL_EVAL_MARKERS = {
    'removable_obstacle_marker': {
        ...
        'model_uri': 'model://room315_visual_obstacle_marker',
        'visual_ids': [
...
def _validated_model_input(model_input: dict[str, Any]) -> dict[str, Any]:
    if not model_input_is_clean(model_input):
        raise ValueError(
            'Room 315 visual-state model_input boundary violation: expected only '
...
class Room315VisualStateDatasetRecorder(Node):
    def __init__(self) -> None:
        super().__init__('room_315_visual_state_dataset_recorder')

        self.declare_parameter('dataset_dir', '~/.ros/room315_visual_state_datasets/demo')
        self.declare_parameter('image_topic', '')
        self.declare_parameter('right_image_topic', '/room_315/perception/right_rail_rgbd/image')
        self.declare_parameter('left_image_topic', '/room_315/perception/left_rail_rgbd/image')
        self.declare_parameter('status_topic', '/room_315/rail_safety/status')
        self.declare_parameter('goal_topic', '/room_315/visual_dataset/goal_text')
        self.declare_parameter('command_topic', '/room_315/rail_safety/primitive_command')
        self.declare_parameter('control_topic', '/room_315/visual_dataset/episode_control')
        self.declare_parameter('recorder_status_topic', '/room_315/visual_dataset/status')
        self.declare_parameter('identity_tracks_topic', '/room_315/perception/privileged_shuttle_identity_tracks')
        ...
        self.get_logger().info(
            f'Room 315 visual-state dataset recorder ready: {self.dataset_dir}'
        )
    ...
    def _start_episode(self, goal: str) -> None:
        ...
        self.last_error = ''
        self.get_logger().info(
            f'Started visual-state dataset episode {self.episode_index}: {goal}'
        )
    ...
    def _stop_episode(self, *, success: bool | None, discarded: bool) -> None:
        ...
        summary = {
            ...
            'success': success,
            'discarded': discarded,
            'format': 'room315_visual_event_labeled_jsonl_v3',
        ...
        self.get_logger().info(
            f'Stopped visual-state dataset episode {self.episode_index}: '
    ...
    def _write_raw_sample(
        ...
        try:
            ...
            self.get_logger().error(f'Failed to record visual-state dataset sample: {exc}')
            self._publish_status('error')
            return None
    ...
    def _write_dataset_info(self) -> None:
        info = {
            ...
            'training_extractor': 'room_315_visual_training_event_extractor.py',
            ...
            'planning_metadata_note': (
                ...
                'are outside model_input and must not be fed to the learned visual-state '
                'policies.'
            ),
...
def main(args=None) -> None:
    ...
    node = Room315VisualStateDatasetRecorder()
    try:
        rclpy.spin(node)
