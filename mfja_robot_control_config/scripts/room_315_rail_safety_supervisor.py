...
def _decode_room315_primitive_command(
    command: dict[str, Any],
    *,
    rails: dict[str, Any],
    emergency_stop: bool,
...
class Room315RailSafetySupervisor(Node):
    def __init__(self) -> None:
        super().__init__('room_315_rail_safety_supervisor')
        ...
        self.declare_parameter('emergency_stop_topic', '/room_315/rail_safety/emergency_stop')
        ...
        self.declare_parameter('right_camera_info_topic', '/room_315/perception/right_rail_rgbd/camera_info')
        self.declare_parameter('left_camera_info_topic', '/room_315/perception/left_rail_rgbd/camera_info')
        ...
        self.get_logger().info(
            f'Room 315 rail-safety supervisor ready. Command topic: '
    ...
    def _safety_decode_command(self, command: Any) -> dict[str, Any]:
        ...
        if _looks_like_numeric_vector(command):
            return _removed_action_vector_decision(command)
        return _decode_room315_primitive_command(
            command,
            rails=self.rails,
    ...
    def _execute(self, command: dict[str, Any] | list[Any]) -> None:
        ...
        if action in {'stoppers', 'stopper'}:
            ...
            return
        raise ValueError(f'unknown primitive command action {action!r}')
...
def main(args=None) -> None:
    ...
    node = Room315RailSafetySupervisor()
    try:
        rclpy.spin(node)
