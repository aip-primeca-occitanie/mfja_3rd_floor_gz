...
`~/.ros/room315_vla_obstacles.json` cache by default. This case disables the
external obstacle feature and expects that clean state. Add
`room315_clear_vla_obstacle_pose_cache:=false` only for a reviewed workflow that
...
ros2 launch mfja_3rd_floor_bringup room_315_only.launch.py \
  ...
  room315_vla_dataset_dir:="$ROOM315_CASE_DATASET" \
...
ros2 topic echo --once /room_315/vla/status std_msgs/msg/String
ros2 topic echo --once /room_315/vla/dataset_status std_msgs/msg/String
...
`/room_315/vla/emergency_stop` Boolean topic for emergency-stop testing, and
