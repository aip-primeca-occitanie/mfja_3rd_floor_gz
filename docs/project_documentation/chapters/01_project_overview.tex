...
Supported and optional dependency surfaces &
  ...
  \file{mfja_robot_control_config/scripts/room_315_vla_to_lerobot.py} \\ \rowseparator
...
An optional vision-language-action (VLA) research stack adds paired-camera
visual-state estimation, constrained English task understanding, PDDL/PlanSys2
planning, one-step closed-loop execution, deterministic supervision, effect
verification, dataset generation, model training, artifact promotion, and
fault/acceptance campaigns. These are separate capabilities: enabling the VLA
camera/supervisor branch does not implicitly start learned inference or task
execution.
...
Camera and primitive supervisor topics & Available, disabled by default & Enable the VLA launch branch \\ \rowseparator
