...
\file{mfja_robot_control_config/scripts/room_315_vla_supervisor.py}. \\ \rowseparator
V4 visual runtime &
...
\file{mfja_robot_control_config/config/room_315_vla/visual_state_runtime.yaml}. \\ \rowseparator
...
\file{mfja_robot_control_config/config/room_315_vla/task_execution_runtime.yaml}. \\ \rowseparator
Controlled shutdown &
\file{mfja_3rd_floor_bringup/launch/room_315_floor_common.py};
...
\code{room315_clear_vla_obstacle_pose_cache:=false}.
...
task execution. If attaching to a floor that was launched without VLA, start
\file{room_315_vla_supervisor.launch.py} directly. Do not do that when the
...
With the integrated VLA camera bridge already running, the safe shadow template
is:
...
paired policy
\code{enable_room315_vla_obstacles:=false} with
