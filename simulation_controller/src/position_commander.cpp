...
private:
    ...
    bool processLine(const std::string & line)
    {
        ...
        double init = positions[joint_id - 1] ;
        std::cout << "Current position : " << init << " rad\n";
        positions[joint_id - 1] = init + value;
        ...
        std::cout << "Sent joint_" << joint_id << " -> " << init + value << " rad\n";
        return false;
    }
...
int main(int argc, char * argv[])
{
    ...
    auto node = std::make_shared<real_commander::PositionCommander>();
